        ...
        if (packet.getBody() instanceof PacketBody8Init1) {
            ...
            switch (init1.getStep().getNumber()) {
                ...
                case 3:
                    ...
                    int sourceOffset = solution.length > 64 ? solution.length - 64 : 0;
                    int destinationOffset = solution.length < 64 ? 64 - solution.length : 0;
                    int copyLength = solution.length < 64 ? solution.length : 64;

                    System.arraycopy(
                            solution, sourceOffset,
                            y, destinationOffset,
                            copyLength
                    );
